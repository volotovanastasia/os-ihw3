...
int main(int argc, char *argv[])
{
    ...
    // Генерация случайных чисел и выбор чисел для нового массива
    srand(time(NULL));
    ...
    while (new_num_clients > 1) {
        ...
        for (int i = 0; i < new_num_clients; i++) {
            if (i == new_num_clients - 1 && new_num_clients % 2 == 1) {
                ...
                send(clients[current_round[i * 2 + (random + 1) % 2]].socket, &loser_message, sizeof(loser_message), 0);
                // Закрытие сокета клиента, которого не выбрали
